...
- 변수란? 컴퓨터 메모리 어딘가에 저장되어 있는 객체를 참조하기 위해 사용되는 이름 ->  box

객체(object): 숫자,문자,클래스 등 값을 가지고 있는 모든 것 -> 그냥 쉽게 things로 생각하자 어떠한 것   을 지칭하고 있구나!정도로 생각
...
3.14-3.02 =0.1200~001 실수체계의 표현 때문에 정수가 아닌 실수를 계산 할 때는 이를 꼭 고려하자

절대값으로 표현하고 엄청 작은 수보다 작은지 확인함
...
Escape sequence 탈출하다..?

얘는 다른 의미로 생각해줘 할 때 사용
...
String Interpolation(문자열 사이에 변수를 넣고 싶다) 많이 쓰임

문자열을 변수를 활용하여 만드는 법

%-formatting 거의 대부분 타 프로그래밍 언어에서 쓰임
...
리스트와 튜플의 차이점을 생각하기 자료변경 가능/불가능
...
모든 시퀀스형 즉,순서가 있는 데이터들(ex.리스트,튜플 등) 은 패킹/언패킹 연산자*를 사용하여 패킹 언패킹이 가능함
...
암시적 형 변환 파이썬 내부적으로 자료형을 변환

명시적 형 변환 사용자가 직접 의도적으로 자료형을 변환 

True를 1로 바꿔서 계산해줌 오ㅐ?편하라고

3은 int고 5.0은 float 바꿔서 계산해줌
...
논리 연산자 단축평가  결과가 확실한 경우 두번째 갑은 확인하지 않고 첫번째  값 반환!

and 모두 True여만함

or 하나라도 True면 됨

and는 둘다 True여야함 5가 True인걸 봤는데 뒤에꺼까지 봐야함 그래서 4가 나옴

or 는 둘중 하나면 됨 5에서 True인걸 확인했으니 5

0이 False인게 확정 되었으니까 0

5에서 이미 True 이니까 뒤에 볼 필요 없음
